...
在线对战支持断线重连功能，当玩家意外断线后，可以在 60 秒内重新连接并恢复游戏状态。

#### 重连场景

1. **页面刷新**：玩家刷新浏览器页面
2. **网络波动**：网络不稳定导致短暂断线
3. **浏览器崩溃**：浏览器意外关闭后重新打开
...
页面加载/断线检测
      ↓
检查 LocalStorage 中的房间信息
      ↓
是否有保存的房间信息？
  ├─ 是 → 恢复游戏状态（gameMode、myColor、isHost 等）
  │       ↓
  │   注册消息处理器（setupWSMessageHandler）⭐ 关键
  │       ↓
  │   调用自动重连（wsStore.autoReconnect()）
  │       ↓
  │   检查 WebSocket 连接状态
  │       ├─ 已连接 → 直接发送 rejoin_room 消息
  │       └─ 未连接 → 先连接 WebSocket，再发送 rejoin_room
  │               ↓
  │           发送 rejoin_room 消息（包含 userId）
  │               ↓
  │           服务器处理重连请求
  │               ↓
  │           服务器发送 rejoined 消息（包含完整游戏状态）
  │               ↓
  │           前端接收并处理 rejoined 消息
  │               ↓
  │           恢复游戏状态并进入对局页面
...
**关键点**：
- ✅ 必须在 `autoReconnect()` 之前调用 `setupWSMessageHandler()`
- ✅ 恢复所有必要的游戏状态
- ✅ 使用 `userId` 而不是 `playerName` 进行身份识别
...
**关键点**：
- ✅ 恢复完整的游戏状态（棋盘、玩家信息、走棋记录等）
- ✅ 设置 `currentScreen.value = 'game'` 进入对局页面
- ✅ 更新房间信息到 LocalStorage
- ✅ 显示成功提示
...
function handleRejoinRoom(ws, data) {
  ...
  // ⭐ 重要：使用 userId 而不是 playerName 匹配玩家
  let playerIndex = -1;
  ...
  if (playerIndex === -1) {
    send(ws, { type: 'error', message: '未找到玩家' });
    return;
  }
...
**关键点**：
- ✅ 使用 `userId` 而不是 `playerName` 匹配玩家
- ✅ 发送完整的游戏状态（棋盘、玩家信息、走棋记录等）
- ✅ 通知对手重连成功
- ✅ 清除重连定时器
...
**1. 消息处理器注册时机**
- ⭐ **必须在发送重连请求之前注册消息处理器**
- 否则无法接收到服务器的 `rejoined` 响应
- 正确做法：先调用 `setupWSMessageHandler()`，再调用 `wsStore.autoReconnect()`

**2. 使用 userId 而不是 playerName**
- `userId` 是唯一的，不会重复
- `playerName` 可能重复（如多个玩家都叫"玩家"）
- 服务器通过 `userId` 匹配玩家位置

**3. 保存完整的房间信息**
- 必须保存 `gameMode`，否则刷新后无法判断是否需要重连
- 必须保存 `myColor`，否则刷新后显示错误的颜色
- 必须保存 `isHost`，否则刷新后无法正确显示界面
...
**5. 状态清理的完整性**
- 房主离开：显示等待面板，保留房间
- 加入者离开：房间已删除，清理所有状态并返回菜单
- 必须清理 LocalStorage 避免状态污染

**6. 消息格式的完整性**
- 房主离开时必须包含 `players` 字段
- 加入者离开时也必须包含 `players` 字段
- 前端根据 `players` 字段同步玩家状态

#### 常见问题

**Q1: 刷新后无法进入对局页面？**
A: 检查是否在 `autoReconnect()` 之前调用了 `setupWSMessageHandler()`。

**Q2: 重连后显示错误的颜色？**
A: 检查是否正确保存和恢复了 `myColor` 字段。

**Q3: 重连后对手看不到我重连？**
A: 检查服务器是否发送了 `opponent_reconnected` 消息给对手。

**Q4: 重连超时后无法操作？**
A: 检查是否清理了 LocalStorage 中的房间信息。

**Q5: 对手退出后无法创建房间？**
A: 检查 `opponent_left` 消息处理中是否清理了房间信息。
...
#### 1. WebSocket 循环引用问题修复

**问题描述：**
- Docker 环境下，玩家加入房间时出现"消息格式错误"
- 玩家能进入房间并发送消息，但房主无法进入页面
- GitHub Pages 和 Railway 环境正常

**根本原因：**
```javascript
// server.js handleJoinRoom 函数
send(room.players[0].ws, {
  type: 'opponent_joined',
  opponent: room.players[1],  // ❌ 包含 ws 属性，导致循环引用
});
```
...
**影响范围：**
- `handleJoinRoom` 函数（两处：向玩家发送 `room_joined`，向房主发送 `opponent_joined`）

**关键要点：**
- 任何时候发送玩家对象时，都不能包含 `ws` 属性
- 参考 `handleRejoinRoom` 的正确实现方式

---

#### 2. Docker WebSocket 代理配置

**问题：**
- Docker 部署时，IP 访问无法连接 WebSocket
- 本地开发时需要手动添加 `?ws=ws://localhost:8080` 参数
...
**问题：**
- 发送 emoji 时，自己的消息同时出现在左边（对手）和右边（自己）
- 发送文本消息时正常

**根本原因：**
```javascript
// App.vue handleSendEmoji 函数
function handleSendEmoji(emoji: string) {
  showEmojiPopup.value = false;
  if (gameMode.value !== 'online') return;
  gameStore.addChatMessage(emoji, 'me', 'emoji');  // ❌ 手动添加一次
  wsStore.sendEmoji(emoji);  // 服务器广播回来又会添加一次
}
```

**解决方案：**
```javascript
// 移除手动添加，只由服务器广播的消息来显示
function handleSendEmoji(emoji: string) {
  showEmojiPopup.value = false;
  if (gameMode.value !== 'online') return;
  wsStore.sendEmoji(emoji);
...
1. **WebSocket 消息格式**：避免包含循环引用（如 `ws` 属性）
2. **重连机制**：使用 `userId` 而不是 `playerName` 识别玩家
3. **状态管理**：保存 `gameMode` 到 localStorage，确保页面刷新后正确恢复
4. **错误处理**：房间不存在时清理 localStorage 并返回菜单
5. **用户体验**：重连失败时显示友好提示
6. **Docker 部署**：使用 nginx/Vite 代理 WebSocket 连接
7. **消息机制**：emoji 广播给所有人，根据 `playerColor` 判断显示位置
...
**代码位置**：`frontend/src/workers/ai.ts:313-330`
...
**代码位置**：`frontend/src/workers/ai.ts:333-350`
...
**代码位置**：`frontend/src/workers/ai.ts:206-216`
...
**代码位置**：`frontend/src/workers/ai.ts:263-284`
...
**结论**：✅ 攻守平衡性设计优秀，根据局势动态调整，三个难度差异化明显。
...
**代码位置**：`frontend/src/workers/ai.ts:185-196`
...
| 棋型 | 分数 | 说明 |
|------|------|------|
| 连五 | 10,000,000 | ✅ 最高优先级，必胜 |
| 活四 | 1,000,000 | ✅ 高威胁，必胜或必防 |
| 冲四 | 300,000 | ✅ 强攻击，能赢或需防守 |
| 跳四 | 150,000 | ✅ 高威胁 |
| 活三 | 15,000 | ✅ 中等威胁，进攻机会 |
| 跳三 | 8,000 | ✅ 中等威胁 |
| 眠三 | 1,000 | ✅ 低威胁 |
...
**代码位置**：`frontend/src/workers/ai.ts:1412-1434`
...
**代码位置**：`frontend/src/workers/ai.ts:411-530`

**特点**：
- 覆盖前 10 步
- 包含 9 种定式（花月、浦月、云月、雨月、松月、秋月、新月、残月、金星）
- 优先检测威胁，威胁优先于定式
...
**代码位置**：`frontend/src/workers/ai.ts:1592-1674`
...
checkEndgameWin(board: number[][], player: Player): Position | null {
  ...
  for (const move of candidateMoves) {
    ...
    if (score >= this.patternScores.RUSH_FOUR + this.patternScores.LIVE_THREE) {
      console.log('AI: 终局检测到对手冲四+活三，必须防守', move);
      return move;
    }
  }
...
**问题1：防守权重冲突** ❌ 已解决
- **问题描述**：简单难度防守权重 0.9，但动态调整可能导致防守不足
- **解决方案**：动态调整基于基础权重，0.9 × 0.7 = 0.63（最低），仍有一定防守能力

**问题2：评分优先级冲突** ❌ 已解决
- **问题描述**：活四评分 1,000,000，但防守活四时评分为 `LIVE_FOUR + 1,000,000 = 2,000,000`
- **解决方案**：防守活四获得更高优先级，符合预期

**问题3：威胁检测重复** ❌ 已解决
- **问题描述**：`checkOpeningBook`、`evaluateMoves`、`checkEndgameWin` 都检测威胁
- **解决方案**：分工明确，`checkOpeningBook` 处理前 10 步，`checkEndgameWin` 处理终局，`evaluateMoves` 处理中局

**问题4：搜索深度冲突** ❌ 已解决
- **问题描述**：简单难度最大深度 3，但终局增加深度 +1
- **解决方案**：终局增加深度是合理的，因为终局分支因子小
...
**AI 三个难度和攻守平衡性设计优秀，无需修改。**

**优点**：
1. ✅ 三个难度差异化明显，适合不同水平玩家
2. ✅ 攻守平衡性动态调整，适应性强
3. ✅ 评分系统准确，棋型识别完整
4. ✅ 开局库和终局检测完善
5. ✅ 无逻辑冲突，代码质量高
...
- **更新日期**：2026-04-10
- **影响版本**：v5.2.6+
- **向后兼容**：是（localStorage 新增字段不影响旧数据）
